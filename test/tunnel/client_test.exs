defmodule ExAliyunOtsTest.Tunnel do
  ...
  setup_all do
    ...
    on_exit(fn ->
      {:ok, _response} = Client.delete_tunnel(@instance_key, table_name: @table_name, tunnel_name: @tunnel_name)
    ...
    for i <- 1..@test_records_size do
      TunnelData.write(
        @table_name,
        [{"id", i}],
        [{"attr", "attr#{i}"}, {"attr2", "somevalue#{i}"}, {"attr3", i}, {"attr4", 89.1}, {"attr5", false}, {"attr6", "#{i}sometestvalue"}]
      )
    end
  ...
  test "describe tunnel", context do
    ...
    {:ok, response} = Client.describe_tunnel(@instance_key, table_name: @table_name, tunnel_name: tunnel.tunnel_name, tunnel_id: tunnel.tunnel_id)
  ...
  test "connect and heartbeat tunnel", context do
    ...
    {:ok, response} = Client.describe_tunnel(@instance_key, table_name: @table_name, tunnel_name: tunnel.tunnel_name, tunnel_id: tunnel_id)
    Logger.info "describe tunnel response: #{inspect response}"
    [channel] = response.channels
    ...
    {shutdown_result, response} = Client.shutdown_tunnel(@instance_key, tunnel_id: tunnel.tunnel_id, client_id: client_id)
