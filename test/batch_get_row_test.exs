  ...
  test "batch get row" do
    ...
    for partition_key <- 1..3 do
      var_put_row = %Var.PutRow{
        ...
        attribute_columns: [{"product_name", "pn_#{partition_key}"}, {"size", partition_key}, {"value", 1.1}],
        condition: condition,
    ...
    for partition_key <- 10..13 do
      var_put_row = %Var.PutRow{
        ...
        attribute_columns: [{"size", partition_key * 10}, {"value", 10.1}, {"name", "name_#{partition_key}"}],
        condition: condition,
        return_type: ReturnType.pk
      }
      {:ok, _result} = Client.put_row(@instance_key, var_put_row)
